...
@test "Rollback Helm repository release" {
  ...
  kubectl patch -f "$FIXTURES_DIR/releases/helm-repository.yaml" --type='json' -p='[{"op": "replace", "path": "/spec/values/replicaCount", "value":"faulty"}]' >&3
  ...
  # Apply fix patch
  kubectl patch -f "$FIXTURES_DIR/releases/helm-repository.yaml" --type='json' -p='[{"op": "replace", "path": "/spec/values/replicaCount", "value":"3"}]' >&3
...
@test "Rollback git release" {
  ...
  # Apply fix patch
  kubectl patch -f "$FIXTURES_DIR/releases/git.yaml" --type='json' -p='[{"op": "replace", "path": "/spec/values/replicaCount", "value":"3"}]' >&3
