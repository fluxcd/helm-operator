...
type HelmValues struct {
	// Values holds the configuration keys and values.
	// Work around for https://github.com/kubernetes-sigs/kubebuilder/issues/528
	Values map[string]interface{} `json:"-"`
}

// MarshalJSON marshals the Values to a JSON blob.
...
type HelmReleaseSpec struct {
	...
	// HelmValues holds values the for this Helm release.
	// +optional
	// +kubebuilder:pruning:PreserveUnknownFields
	HelmValues `json:"values,omitempty"`
}
